...
def edit_new_labels(existing_path, new_path):
    """
    This is used to correct the bounding boxes generated by Gemini.
    The bounding boxes are divided by 2 to get the correct values since
    they were not divided by 2 when generated.
    """
    existing_labels = os.listdir(existing_path)

    for label_name in os.listdir(new_path):
        if label_name not in existing_labels:
            continue

        # Open the new label and divide the first two entries by 2
        with open(os.path.join(new_path, label_name), "r") as f:
            lines = f.readlines()
        ...
        for line in lines:
            ...
            new_line = f"{parts[0]} {x} {y} {w} {h}\n"
            new_lines.append(new_line)

        with open(os.path.join(new_path, label_name), "w") as f:
    ...
    for i in tqdm(range(start, end)):
        ...
        if bboxes_and_names is not None and len(bboxes_and_names) != 0:
            for bbox_and_name in bboxes_and_names:
                ...
                all_bboxes.append([(x1+x2)/2/width, (y1+y2)/2/height, (x2-x1)/width, (y2-y1)/height])
...
def combine_gemini_and_coco():
    ...
    """
    coco_labels_path = "data/train/coco_labels"
    gemini_labels_path = "data/train/gemini_labels_full"
    combined_labels = "data/train/labels"
    images = "data/train/images"

    coco_label_names = os.listdir(coco_labels_path)
    gemini_label_names = os.listdir(gemini_labels_path)
    ...
    for image_name in tqdm(image_names):
        ...
        coco_label_path = os.path.join(coco_labels_path, coco_label_name)
        ...
        gemini_label_path = os.path.join(gemini_labels_path, gemini_label_name)
...
if __name__ == "__main__":
    ...
    # Used to correct the bounding boxes generated by Gemini (divide by 2)
    # Only use if the bounding boxes are not divided by 2    
    # edit_new_labels("../OSOD-LLM/yolo_data/OWDETR/task1/train/labels_gemini_full", "data/train/gemini_labels_full")
