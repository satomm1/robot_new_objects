...
"""
This script trains the YOLOv11 model on the dataset
"""
...
if __name__ == "__main__":
    ...
    data_dir = os.path.join(home_dir, "data")  # Path to the data directory
    settings.update({"datasets_dir": data_dir})  # Update the datasets directory

    # Remove the labels cache files
    if os.path.exists("data/train/labels.cache"):
        os.remove("data/train/labels.cache")
    if os.path.exists("data/val/labels.cache"):
        os.remove("data/val/labels.cache")    
    ...
    # Begin training
    model.train(data="tools/data.yaml", epochs=epochs, batch=batch_size, imgsz=imgsz)
